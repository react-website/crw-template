...
var pJS = function (tag_id, params) {
    ...
    this.pJS = {
        canvas: {
            ...
            number: {
                ...
                value: 20, random: false, anim: {
                    enable: false, speed: 20, size_min: 0, sync: false
                }
            }, line_linked: {
    ...
    pJS.fn.particle = function (color, opacity, position) {
        ...
        if (typeof (color.value) == 'object') {
            ...
            if (color.value instanceof Array) {
                ...
                if (color.value.h != undefined && color.value.s != undefined && color.value.l != undefined) {
                    this.color.hsl = {
                        h: color.value.h, s: color.value.s, l: color.value.l
                    }
                }
            }
        ...
        if (this.shape == 'image') {
            ...
            this.img = {
                src: sh.image.src, ratio: sh.image.width / sh.image.height
            }
    ...
    pJS.fn.particlesUpdate = function () {
        ...
        for (var i = 0; i < pJS.particles.array.length; i++) {
            ...
            if (pJS.particles.move.out_mode == 'bounce') {
                var new_pos = {
                    x_left: p.radius, x_right: pJS.canvas.w, y_top: p.radius, y_bottom: pJS.canvas.h
                }
            } else {
                var new_pos = {
                    x_left: -p.radius,
    ...
    pJS.fn.modes.repulseParticle = function (p) {
        ...
        if (pJS.interactivity.events.onhover.enable && isInArray('repulse', pJS.interactivity.events.onhover.mode) && pJS.interactivity.status == 'mousemove') {
            ...
            var normVec = {x: dx_mouse / dist_mouse, y: dy_mouse / dist_mouse},
                repulseRadius = pJS.interactivity.modes.repulse.distance, velocity = 100,
                repulseFactor = clamp((1 / repulseRadius) * (-1 * Math.pow(dist_mouse / repulseRadius, 2) + 1) * repulseRadius * velocity, 0, 50);

            var pos = {
                x: p.x + normVec.x * repulseFactor, y: p.y + normVec.y * repulseFactor
            }
            ...
            if (pJS.tmp.repulse_clicking) {
                ...
                var dx = pJS.interactivity.mouse.click_pos_x - p.x, dy = pJS.interactivity.mouse.click_pos_y - p.y,
                    d = dx * dx + dy * dy;
...
const particlesJS = function (tag_id, params) {
    ...
    /* pJS elements */
    var pJS_tag = document.getElementById(tag_id), pJS_canvas_class = 'particles-js-canvas-el',
